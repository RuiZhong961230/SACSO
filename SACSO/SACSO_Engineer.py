...
def RunSACSO(func):
    ...
    np.savetxt("./SACSO_Data/Engineer/" + str(FuncNum) + ".csv", All_Trial, delimiter=",")
...
def main():
    ...
    Probs = [CBD(), CBHD(), GTD(), PLD(), RCB(), SRD(), TBTD(), TCD()]
    Names = ["CBDP", "CBHDP", "GTDP", "PLDP", "RCBDP", "SRDP", "TBTDP", "TCDP"]
    for i in range(len(Probs)):
        ...
        LB = Probs[i].lb
        UB = Probs[i].ub
        RunSACSO(Probs[i].evaluate)

if __name__ == "__main__":
    if os.path.exists('./SACSO_Data/Engineer') == False:
        os.makedirs('./SACSO_Data/Engineer')
    main()