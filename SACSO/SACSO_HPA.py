...
def mainCons(ndiv, level):
    global FuncNum, DimSize, Pop, MaxFEs, curIter, MaxIter, LB, UB
    ...
    for i in range(len(Probs)):
        ...
        UB = [1] * DimSize
        FuncNum = Name[i]
        Trace = RunSACSO(Probs[i])
        np.savetxt("./SACSO_Data/HPA_Cons/" + FuncNum + "_" + str(ndiv) + "_" + str(level) + ".csv", Trace, delimiter=",")
...
def mainUncons(ndiv, level):
    ...
    for i in range(len(Probs)):
        ...
        FuncNum = Name[i]
        Trace = RunSACSO(Probs[i])
        np.savetxt("./SACSO_Data/HPA_Uncons/" + FuncNum + "_" + str(ndiv) + "_" + str(level) + ".csv", Trace, delimiter=",")


if __name__ == "__main__":
    if os.path.exists('./SACSO_Data/HPA_Uncons') == False:
        os.makedirs('./SACSO_Data/HPA_Uncons')
    if os.path.exists('./SACSO_Data/HPA_Cons') == False:
        os.makedirs('./SACSO_Data/HPA_Cons')
    for ndiv in range(4, 6):
